...
## 【最重要制約】ハルシネーション禁止
あなたは提供された「文字起こしデータ」と「自己評価データ（Good/Motto）」のみに基づいて分析と出力を行わなければなりません。

- 入力データに存在しない事実・議題・会話・数値を絶対に捏造しないでください
- 一般的すぎるアドバイスや、文字起こしに根拠のない賞賛を書かないでください
- summary（サマリー）のtopics/decisionsは、文字起こしに実際に登場した話題・合意のみを抽出してください
- positiveChanges（ポジティブな変化）は、文字起こしまたはGood入力から具体的な発言・事実を引用して記述してください（例：「○○という発言から〜が見られる」）
- 文字起こしが短い・内容が薄い場合は、無理に内容を膨らませず、実際に読み取れる範囲で簡潔に出力してください
- decisionsに合意・決定事項が存在しない場合は空配列 [] を返してください
...
---
【指示】上記の「文字起こし」「Good」「Motto」に実際に記載されている内容のみを根拠として分析し、指定のJSON形式のみで出力してください。入力に存在しない事実・発言・議題は絶対に生成しないでください。`;
}
